...
import { X, Upload, FileText, Trash2, Calendar, User, Phone, Activity } from 'lucide-react'
...
export function AppointmentForm({ patientId, onClose }: AppointmentFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (staffError || !staffMember) {
        toast.error('You must be a member of a clinic to create appointments', { id: loadingToast })
        return
      }
        ...
        if (medicalRecords.length > 0) {
          ...
          for (let i = 0; i < medicalRecords.length; i++) {
            ...
            try {
              ...
              // Upload to storage
              const uploadResult = await uploadMedicalRecord(finalPatientId, compressedFile)
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen p-4 sm:p-6"
    >
      <div className="max-w-5xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            ...
            >
              {loading ? 'Creating...' : uploading ? 'Uploading files...' : 'Create Appointment'}
            </motion.button>
            {onClose && (
              <motion.button
                whileHover={{ scale: 1.02 }}
                ...
                type="button"
                onClick={onClose}
                className="px-8 py-3.5 border-2 border-gray-300 text-gray-700 font-semibold rounded-xl hover:bg-gray-50 transition"
              >
                Cancel
              </motion.button>
            )}
